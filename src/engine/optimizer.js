...
/**
 * Find alternative suppliers when tariffs are imposed on certain countries.
 * Ranks all countries (not in affected list) by their effective cost advantage.
 *
 * @param {string[]} affectedCountryIds - Array of country IDs affected by tariffs
 * @param {string} category - Commodity category key (e.g. "electronics")
 * @param {object} simulatedGraph - Graph with updated tariff rates
 * @param {object} originalGraph - Original graph with baseline tariffs
 * @param {object} weights - { cost, speed, risk } weights
 * @returns {Array} Ranked alternative suppliers with cost savings
 */
export function findTariffAlternatives(affectedCountryIds, category, simulatedGraph, originalGraph, weights) {
  const affectedVolume = simulatedGraph.nodes
    .filter(n => affectedCountryIds.includes(n.id))
    .reduce((sum, n) => sum + (n.export_volumes[category] || 0), 0);

  if (affectedVolume === 0) return [];

  const maxVolume = Math.max(
    ...simulatedGraph.nodes.map((n) => n.export_volumes[category] || 0)
  );

  const available = simulatedGraph.nodes.filter(
    (n) => !affectedCountryIds.includes(n.id) && (n.export_volumes[category] || 0) > 0
  );
  ...
  const scored = available.map((node) => {
    ...
    const coveragePct = (node.export_volumes[category] || 0) / affectedVolume;

    // Calculate cost savings vs the lowest-cost affected country
    const affectedCosts = simulatedGraph.nodes
      .filter(n => affectedCountryIds.includes(n.id))
      .map(n => (n.tariff_rates[category] || 0) + n.distance_cost_factor);
    const minAffectedCost = Math.min(...affectedCosts);
    const altCost = (node.tariff_rates[category] || 0) + node.distance_cost_factor;
    const costSavings = minAffectedCost - altCost;
